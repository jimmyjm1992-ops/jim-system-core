...
def classify_trend_4h(closes):
    # bullish if EMA21 > EMA50 and close > EMA21 and MACD>0 and RSI>50
    e21 = ema(closes, 21)
    e50 = ema(closes, 50)
    r = rsi(closes, 14)
    m, s, h = macd(closes)
...
def classify_zone_1h(highs, lows, closes, pct=0.3):
    # Donchian 20. If close within pct% of upper/lower → breakout/breakdown zone
...
def retest_signal(bias4h, highs1h, lows1h, closes1h):
    # Simple retest doctrine:
    # Bull: 1H closed above Donchian high and current price is within ±0.3% of that high -> LONG retest band
    # Bear: 1H closed below Donchian low and current price is within ±0.3% -> SHORT retest band
